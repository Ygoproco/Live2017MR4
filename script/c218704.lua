...
function c218704.chkfilter(c,ft,sg,rg)
	local res
	if sg:GetCount()<2 then
		sg:AddCard(c)
		res=rg:IsExists(c218704.chkfilter,1,sg,ft,sg,rg)
		sg:RemoveCard(c)
	else
		res=sg:FilterCount(c218704.mzfilter,nil)+ft>0
	end
	return res
end
function c218704.mzfilter(c)
...
function c218704.spcon(e,c)
	...
	return ft>-2 and rg:GetCount()>1 and rg:IsExists(c218704.chkfilter,1,nil,ft,Group.CreateGroup(),rg)
end
function c218704.spop(e,tp,eg,ep,ev,re,r,rp,c)
	...
	while g:GetCount()<2 do
		...
		local tc=rg:Filter(c218704.chkfilter,g,ft,g,rg):SelectUnselect(g,tp)
