...
function c511002323.chkfilter(c,ft,sg,rg,e,tp)
	local res
	if sg:GetCount()<4 then
		sg:AddCard(c)
		res=rg:IsExists(c511002323.chkfilter,1,sg,ft,sg,rg,e,tp)
		sg:RemoveCard(c)
	else
		res=sg:FilterCount(c511002323.mzfilter,nil)+ft>0 and Duel.IsExistingMatchingCard(c511002323.spfilter,tp,LOCATION_GRAVE,LOCATION_GRAVE,1,sg,e,tp)
	end
	return res
end
function c511002323.mzfilter(c)
...
function c511002323.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then 
		if chkcost then
			e:SetLabel(0)
			return ft>-4 and rg:GetCount()>3 and rg:IsExists(c511002323.chkfilter,1,nil,ft,Group.CreateGroup(),rg,e,tp)
		else
	...
	if chkcost then
		...
		while g:GetCount()<4 do
			...
			local tc=rg:Filter(c511002323.chkfilter,g,ft,g,rg,e,tp):SelectUnselect(g,tp)
