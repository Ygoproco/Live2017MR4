...
function c62543393.chkfilter(c,ft,sg,rg)
	local res
	if sg:GetCount()<2 then
		sg:AddCard(c)
		res=rg:IsExists(c62543393.chkfilter,1,sg,ft,sg,rg)
		sg:RemoveCard(c)
	else
		res=sg:FilterCount(c62543393.mzfilter,nil)+ft>0
	end
	return res
end
function c62543393.mzfilter(c)
...
function c62543393.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return ft>-2 and rg:GetCount()>1 and rg:IsExists(c62543393.chkfilter,1,nil,ft,Group.CreateGroup(),rg) end
	local g=Group.CreateGroup()
	while g:GetCount()<2 do
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
		local tc=rg:Filter(c62543393.chkfilter,g,ft,g,rg):SelectUnselect(g,tp)
